...
//				session.invalidate();
	...
	public ModelAndView MagazineView(ModelAndView mv, String ob) {
//		int listCount = uService.getListCount3();
		...
		ArrayList<Magazine> mList = null; 
		
			
		 mList = uService.selectmList(ob);
	...
	public ModelAndView magazineDetail(ModelAndView mv, int m_NO) {
		Magazine m = uService.selectMagazine(m_NO);
...
//		m.setM_Comment(m.getM_Comment().replace("\r\n","<br>"));
	 ...
	 * @ResponseBody
	 * 
	 * @RequestMapping("orderby") public String orderby(String ob){ // int result =
	 * uService.countMonth(ob);
	 * 
	 * switch(ob) { ob == }
	 * 
	 * 
